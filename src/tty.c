    ...
    if (previous_char == KEY_CTRL_T)
    {
        switch (input_char)
        {
            ...
            case KEY_C:
		        ...
		        tio_printf(" Endline: %s", option.endline);
    ...
    while (true)
    {
        ...
        if (status > 0)
        {
            ...
            if (FD_ISSET(STDIN_FILENO, &rdfs))
            {
                ...
                if (input_char == KEY_CTRL_T) forward = false;
                ...
                if (forward)
                {
		            /*Send data pack*/
		            if (output_char == 13){                               
						if (strcmp("yes", option.endline) == 0){
							//warning_printf("Poszlo z endline");			
		    	        	unsigned char zn = '\r';
							tx_buff[buffsize] = zn;
						}
					else {
			    ...
			    while(x<buffsize+1){
				    ...
				    status = write(fd, &znak, 1);
                    //warning_printf("Tx: %d",znak);
		            if (status < 0)  warning_printf("Could not write to tty device");
